...
# /*
# ------------------------------------------
# @Description: 腾讯地图 - 小程序签到/任务（YYB code / smallcat 兼容）
# cron: 34 18 * * *
# ------------------------------------------
# 变量名：txdt
# 变量值：YYB 地址@账号ID或OpenID，多账号换行；或 openid（视脚本）
#
# 依赖变量：
# YYB_SERVER       推荐，每行 地址@账号ID
# wx_server_url    可选，smallcat 地址（勿写进仓库）
# wx_auth          可选，smallcat 鉴权
# txdt_appid  可选
# QL_NOTIFY        可选，设为 0 关闭推送
# 注：密钥一律环境变量；推送走 send_notify；日志统一模板
# ------------------------------------------
# */
...
TMAP_SECRET = os.getenv("SCRIPT_SECRET", "").strip()  # 脱敏：勿写进仓库
CHECKIN_TOKEN = os.getenv("SCRIPT_TOKEN", "").strip()  # 脱敏
...
def _ql_finish(task_name, accounts, start_ts=None):
    ...
    if format_report and _sn:
        try:
            body = format_report(task_name, accounts)
            pr = _sn(task_name, body)
