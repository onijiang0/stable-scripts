...
# @Author: onijiang0
# @Date: 2026.09.21
# @Description: 荷叶健康 - 果园打卡/浇水/任务/签到抽奖
# cron: 27 16 * * *
# #定时使用10-19点 随机时间 每天
# ------------------------------------------
# 变量名：hyjk
...
# wx_auth           必填，取码服务鉴权
# HYJK_CHANNEL_CODE 选填，渠道码，默认 130
# HYJK_BROWSE_SECONDS 选填，浏览任务兜底秒数
# QL_NOTIFY         选填，0 关闭推送
# ------------------------------------------
# 已实现：
# 1. 多账号；缺变量报错；单号失败不中断
# 2. code 换 token；本地缓存；失效删缓存后 code 重登并重跑
# 3. 果园信息/打卡/浇水（RSA secret）；任务领水；活动签到
# 4. send_notify 统一简报；平台 RSA 密钥写在脚本默认值
#
# 契约（appid wx776afedbfae3a228，tuan.api.ybm100.com）：
# code     POST {wx_server_url}/wx/code auth:{wx_auth} json:{openid,appid}
# 登录     POST /miniapp/login  body {code,appid,channelCode} -> token
#          （源脚本注明登录为推断端点，失败需抓包核对）
# 果园     GET  /api/healthSquare/fruitManor/getMyManorInfo?channelCode=
# 打卡     POST /miniapp/marketing/signActivity/signRecord|sign  sceneId=6
# 活动签到 POST signRecord|sign  actId=9093
# 浇水     POST /api/healthSquare/water/watering  body+query secret=RSA
# secret   SHA1withRSA(参数排序 k=v&) + RSA 长加密(sign&timestamp) URL encode
# 任务     getTaskListNew / userOperation / collectWater 等，按原脚本
#
# 踩坑：
# 1. 原脚本有 token 缓存：失效必须删缓存再 code 重登后重跑
# 2. RSA 私钥/公钥来自源脚本逆向，属平台侧参数，写在脚本内
# 3. 登录接口为推断，失败时看日志 msg，必要时抓包改 LOGIN_URL
# 4. 账号相关只改环境变量；脚本不含个人 openid
# ------------------------------------------
# */
...
try:
    ...
    PKCS1_v1_5 = SHA1 = RSA = pkcs1_15 = None

APP_NAME = "荷叶健康"
APPID = "wx776afedbfae3a228"

# 账号：hyjk = openid，多账号换行或 &
...
def json_preview(data: Any, limit: int = 80) -> str:
    try:
...
def send_pushplus(title: str, content: str) -> None:
    print("🔔 推送结果：已交由 send_notify/面板推送通道处理")
...
def get_code(openid: str) -> str | None:
    ...
    try:
        ...
        print(f"❌ 取码异常: {clean_line(exc)}")
        return None
    if not data.get("status"):
        print(f"❌ 取码失败: {data.get('message') or data.get('code')}")
        return None
...
def auth_related_error(msg: str) -> bool:
    if re.search(r"渠道|channel|RSA|果园|任务", msg) and not re.search(r"token|登录|未登录|401", msg, re.I):
        return False
    return bool(re.search(r"token|登录失效|未登录|请先登录|401", msg, re.I))


def run_one_openid(openid: str, index: int, total: int) -> Dict[str, Any]:
    extras = [f"openid：{mask_id(openid, 6)}"]
    acc = {
        "account": f"账号{index}",
        "phone": "",
        "status": "-",
    ...
    say = lambda m: print(clean_line(m) or "")
    say(f"━━━━━━━━━━━━━━━━━━━━\n👤 账号 {index}/{total} {mask_id(openid, 6)}\n━━━━━━━━━━━━━━━━━━━━")

    def _biz(res: dict) -> dict:
        sign = res.get("signMsg") or "-"
        garden = res.get("gardenMsg") or "-"
        task = res.get("taskMsg") or "-"
        acc["status"] = f"{sign} ✅" if res.get("success") and "失败" not in sign else sign
        if res.get("success"):
            acc["status"] = f"{sign}" if "✅" in sign or "已签" in sign or "成功" in sign else f"{sign} ✅"
            acc["success"] = True
        extras.append(garden)
        extras.append(task)
        # 水滴作为收益展示
        m = re.search(r"水滴[:：]\s*([0-9.\-]+)", garden)
        if m:
            acc["reward"] = f"水滴 {m.group(1)}"
        if not res.get("success"):
            acc["error"] = clean_line(res.get("error") or sign)
        return acc

    def _fail(msg: str, status: str) -> dict:
        acc["status"] = status
        acc["error"] = msg
        acc["success"] = False
        return acc
    ...
    try:
        res = run_account(index, total, openid)
        if res.get("success"):
            extras.append("token缓存登录" if "缓存" in str(res) else "code登录")
            return _biz(res)
        err = clean_line(res.get("error") or "执行失败")
        if auth_related_error(err):
            say("⚠️ 登录态失效，删除缓存 → code 重登 → 重跑")
            extras.append("登录态失效 code重登")
            try:
                clear_cached_token(openid) if "clear_cached_token" in globals() else None
            except Exception:
                pass
            # 原 login_with_cache 已在失效时重新 code 登录
            res2 = run_account(index, total, openid)
            return _biz(res2)
        return _fail(err, f"失败 ❌ ({err[:40]})")
    except Exception as e:
        msg = clean_line(e)
        say(f"❌ {msg}")
        return _fail(msg, f"异常 ❌ ({msg[:40]})")


def main() -> int:
    started = time.time()
    openids = split_openids(os.getenv("hyjk", "").strip())
    if not openids:
        print("❌ 未配置 hyjk 环境变量（openid，多账号换行或 &）")
        return 1
    ...
    for i, openid in enumerate(openids, 1):
        try:
            results.append(run_one_openid(openid, i, len(openids)))
        except Exception as e:
            results.append({
                "account": f"账号{i}",
                "phone": "",
